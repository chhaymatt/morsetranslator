...
describe("Translating Morse to English correctly", () => {  
    ...
    test("Test Morse 2", () => {
        expect(morseToEnglish(testData[2], dictionary, false)).toBe("HELLO THERE!");
    })
    test("Test Morse 3", () => {
        expect(morseToEnglish(testData[3], dictionary, false)).toBe("123 456 890");
    })
    test("Test Morse 4", () => {
        expect(morseToEnglish(testData[4], dictionary, false)).toBe("ABCDEFGHIJKLMNOPQRSTUVWXYZ");
    })
    test("Test Morse 5", () => {
        expect(morseToEnglish(testData[5], dictionary, false)).toBe("HTTPS://GOOGLE.COM.AU");
    })
    test("Test Morse 6", () => {
        expect(morseToEnglish(testData[6], dictionary, false)).toBe("CHHAYMATT@GMAIL.COM");
    })
    test("Test Morse 7", () => {
        expect(morseToEnglish(testData[7], dictionary, false)).toBe("-PPLE.. ..B-N-N- C-.-RROT");
    })
    test("Test Morse 8", () => {
        expect(morseToEnglish(testData[8], dictionary, true)).toBe("THIS IS A TEST MESSAGE!");
    })
})